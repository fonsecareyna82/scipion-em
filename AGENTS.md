...
- `pwem/protocols/protocol_import/` — `ProtImportParticles` (and, going forward, other `ProtImportFiles`-based import protocols) build their `importFrom` menu and per-format form fields from `Domain.findCapabilityProviders('import', self.getClass())` (`pyworkflow.plugin.Domain`/`pyworkflow.capability.CapabilityProvider` — see scipion-pyworkflow's `.ai/capability-providers.md`) instead of a hardcoded `IMPORT_FROM_*` list — a plugin self-registers via the `pyworkflow.capability_provider` entry-point group and only shows up if actually installed. `import_providers.py` in this same directory holds **pwem-native wrapper providers** (`emx`/`xmipp3`/`relion`/`frealign`/`eman`/`scipion`) for formats whose own plugin repo hasn't migrated to self-registering yet — each is a thin adapter around the same `Domain.importFromPlugin(...)` call the old `elif` chain made, gated by `isAvailable()` probing whether the wrapped plugin is actually importable. `importFrom` itself is a `pyworkflow.protocol.params.KeyedEnumParam` (string-key value), not `EnumParam` — this pattern is generic (see the pyworkflow doc), not import-specific, so any other protocol family can adopt the same template later.

...
- `pwem/protocols/protocol_align_movies.py`'s `ProtAlignMovies.computeThumbnail()` - **[Fixed 2026-08-27, verified against EMAN2's real algorithm]** a second, independent copy of the same anti-pattern: this base class (inherited by every plugin's movie-alignment protocol) shelled out to eman2's `e2proc2d.py --meanshrink --fixintscaling=sane` just to mean-shrink and contrast-normalize a preview image. Also carried a dead, unused duplicate of `ImageHandler`'s `__runXmippProgram` helper (copy-pasted, never called - removed too). Reimplemented in-process with numpy + PIL. EMAN2 isn't installed anywhere in this environment (no pip package, needs its own standalone installer) so this couldn't be verified by running the real thing side-by-side - instead, confirmed the exact algorithm against EMAN2's own source (`eman2/programs/e2proc2d.py` on GitHub): `meanshrink` is `math.meanshrink` (NxN block averaging - matches), `fixintscaling=sane` maps `[mean - 2.5*sigma, mean + 2.5*sigma]` to the output range (the reimplementation's *first* attempt used a percentile-based stretch instead, which was closer to "a reasonable preview" than to EMAN2's actual formula - caught by the user asking how equivalence was verified, corrected to use the real mean±2.5σ formula, and added a test (`test_matchesEman2FixintscalingSaneFormula`) that predicts exact expected output pixel values from the formula and asserts against them, not just "runs and looks plausible"). Confirmed via `renderLabels`/`PSD_LABELS` in `viewers_data.py` that the thumbnail is only ever used as a `_filename` for GUI-table image rendering, never read back for computation - so exact-pixel EMAN2 parity isn't load-bearing for correctness, but matching the real formula (now done) is still the right bar, not an approximation. `ImageHandler.computeThumbnail` (a *different*, still-eman2-coupled method with different args - `--fouriershrink`/`--process normalize`) is a separate, still-open instance of the same problem, tracked among the `ImageHandler` methods above.
